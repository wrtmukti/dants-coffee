...
<div class="row my-3">
  <h2 class="text-center text-dark">Pesanan Reservasi</h2>
  <h4 class="text-center text-secondary  fw-bold">by : {{ $customer->name }}</h4>
</div>
...
@foreach ($orders as $order)
  <div class="row mt-3 justify-content-start">
      <p class="text-success "> dipesan ({{ $order->created_at->diffForHumans(); }})</p>
  </div>
                  ...
                  <div class="card">
                    <div class="row">
                      ...
                      <div class="col-6 p-3">
                        ...
                        <p class="card-text ">Rp {{ number_format($data->price * $data->pivot->quantity, 0, ',', '.') }},-</p>
                      </div>
                    </div>
                  </div>
                </div>
                        ...
                        <form action="/admin/order/{{$order->id}}" method="post">
                          ...
                          <div class="modal-body">            
                            ...
                            <div class="form-group">
                              ...
                              <input class="form-control text-center" id="amount" type="text" name="" value="{{number_format($order->price, 0, ',', '.')  }}" readonly="readonly">
                            </div>
                          </div>
                ...
                <div class="col-6">
                  ...
                  <div class="modal fade" id="penolakan" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                      <div class="modal-content">
                        <div class="modal-header">
                          <h5 class="modal-title" id="exampleModalLabel">Pesanan customer {{ $order->customer->name }}</h5>
                          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <form action="/admin/order/{{$order->id}}" method="post">
                          @csrf
...
@endforeach
  ...
  <div class="row my-5">
    @if ($orders->where('status', '<', '3' )->count() == 0)
      <a type="button" href="/admin/transaction/invoice/{{ $transaction_id }}" class="btn btn-dark text-white">
        Cetak Struk
      </a>
      @else    
